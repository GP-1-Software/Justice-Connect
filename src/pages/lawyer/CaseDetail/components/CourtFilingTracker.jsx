...
const CourtFilingTracker = ({ caseId, caseData }) => {
            ...
            switch (actionId) {
                ...
                case 'mark_fully_executed':
                    ...
                    if (window.confirm('هل أنت متأكد من أن الحكم تم تنفيذه بشكل نهائي؟\n\nسيتم تحويل حالة القضية إلى "منفذة بالكامل" وإرسال إشعارات للعميل.')) {
                        try {
                            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/public/cases/${caseId}/mark-fully-executed`, {
                                method: 'POST',
    ...
    const handleSubmitAppeal = async () => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/public/appeals/submit', {
                method: 'POST',
                headers: getAuthHeaders(),
                body: JSON.stringify({
    ...
    const handleSubmitAppealUpdate = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/public/appeals/${caseAppeal.appeal_id}/submit-update`,
                {
                    method: 'POST',
                    headers: getAuthHeaders(),
                    body: JSON.stringify({
    ...
    const handleUploadDocuments = async () => {
        ...
        try {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/notifications/document-uploaded', {
                    method: 'POST',
                    headers: {
    ...
    const handlePostponeRequest = async () => {
        ...
        try {
            ...
            try {
                await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/notifications/postpone-request', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('token')}`
