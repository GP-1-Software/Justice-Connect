...
const Timeline = ({ updates, caseData, onEventDeleted }) => {
  ...
  const validUpdates = (updates || []).filter(event => event != null);
  ...
  .filter(event => event != null && event.created_at) // Filter out invalid events
        ...
        <div className="space-y-6">
          {allEvents.length === 0 ? (
            ...
            allEvents.map((event, index) => {
              // Safely get event type, default to 'default' if not found
                ...
                <div key={eventId} className="relative flex items-start gap-4 pr-8">
                  ...
                  <div className="flex-1 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                      ...
                      {eventId !== 'case-created' && event?.event_id && event?.author_id === lawyer?.lawyer_id && (
                        <button
