...
const HearingsManagement = () => {
    ...
    const fetchAllHearingsForCases = async () => {
        try {
            ...
            for (const caseItem of cases) {
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/cases/${caseItem.case_id}/hearings`, {
                    headers: getAuthHeaders()
                });
                if (response.ok) {
                    const data = await response.json();
    ...
    const fetchPostponeRequests = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/postpone-requests', {
                headers: getAuthHeaders()
            });
            if (response.ok) {
                const data = await response.json();
    ...
    const fetchHearings = async (caseId) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/cases/${caseId}/hearings`, {
                headers: getAuthHeaders()
            });
            if (response.ok) {
                const data = await response.json();
    ...
    const handleScheduleHearing = async (e) => {
        ...
        try {
            const response = await fetch(
                `${import.meta.env.VITE_API_BASE_URL || 'https://justice-connect-mobile.onrender.com'}/api/court-clerk/cases/${selectedCase.case_id}/hearings`,
                {
                    method: 'POST',
                    headers: getAuthHeaders(),
